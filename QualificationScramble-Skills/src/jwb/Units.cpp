...
namespace jwb{
    ...
    RelativeUnit::RelativeUnit(std::function<double(void)> value, double offset)
        : m_value(value),
        m_offset(offset)
    {
    }
    RelativeUnit::RelativeUnit(std::function<double(void)> value)
        : m_value(value),
        m_offset(0)
    {
        m_offset = m_value();
