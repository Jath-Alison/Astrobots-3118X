...
vex::vision::signature SIG_1 = vex::vision::signature(1, -5163, -3827, -4495, -5767, -3681, -4724, 3.9, 0);
...
vex::vision Vision1 = vex::vision(vex::PORT10, 150, SIG_1, SIG_2, SIG_3, SIG_4, SIG_5, SIG_6, SIG_7);
...
bool driveTowardBall(bool clear)
{
    ...
    smartDrive.arcade(0, 50, pid.calculate(Jath::Degrees(centerXData.getMedian() * 5)));

    if(intake.current() > 3){
        intake.set(0);
        currentData.put(intake.current());
    }
    if(currentData.filled() && currentData.getAverage() > 3){
