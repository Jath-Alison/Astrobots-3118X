...
namespace logging
{
    ...
    int logLoop()
    {
        logger.clearFile("AITestLog.wpilog");
        ...
        while (true)
        {
            ...
            std::vector<double> baseXYRCmd = {
                holoDrive.m_cmdX, holoDrive.m_cmdY, holoDrive.m_cmdRot};
            ...
            if (logger.getDataSize() > 5000 || logTimePassed() > 1.0)
            {
                last_logged = std::chrono::high_resolution_clock::now();
                logger.writeToFile("AITestLog.wpilog");
            }
